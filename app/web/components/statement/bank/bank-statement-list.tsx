...
import { List } from "../list/statement-list";
...
const GetIcon = ({ financialFlowType }: StyleComponentProps): JSX.Element => {
  if (financialFlowType === FinancialFlowType.IN) return <IncomeIcon />;
...
const BankStatementComponent = ({
  ...
  balanceAccount,
  onSelect,
}: BankStatementListProps): JSX.Element => {
  currentValueByMovement = balanceAccount;

  return (
    <List title="Extrato">
      {data.map(
        ({ createdAt, financalCategory, value, id, title, description }) => (
          <div
            key={id}
            onClick={onSelect}
          ...
          >
            <div className="flex gap-4 items-center">
              ...
              >
                <GetIcon
                  financialFlowType={financalCategory.financialFlowType}
                />
              </div>
            ...
            <div className="text-right">
              ...
              >
                {getStatusValue(
                  financalCategory.financialFlowType,
